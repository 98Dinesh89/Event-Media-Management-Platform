    ...
    <div className="min-h-screen bg-[#111111] text-[#F0EDE8]">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5">
          <h1 className="text-lg font-semibold text-[#F0EDE8]">Events</h1>
          {canCreateEvent && (
            <Link
              href="/events/create"
        ...
        <div className="flex flex-col md:flex-row md:items-center gap-2 mb-6">
          <div className="relative flex-1 max-w-full md:max-w-sm">
            ...
            <input
              ...
              className="w-full bg-[#171717] border border-[#2A2622] rounded-md pl-9 pr-4 py-2 text-sm text-[#F0EDE8] placeholder-[#7C7A74] focus:outline-none focus:border-[#F59E0B]"
            />
          </div>
          ...
          <select
            ...
            className="bg-[#171717] border border-[#2A2622] rounded-md px-3 py-2 text-sm text-[#B5B1AA] focus:outline-none focus:border-[#F59E0B]"
          >
              ...
              <Link key={event.id} href={`/events/${event.id}`}>
                <div className="bg-[#171717] border border-[#2A2622] hover:border-[#F59E0B] rounded-md overflow-hidden transition group cursor-pointer">
                  ...
                  <div className="h-40 bg-[#1A1A1A] relative overflow-hidden">
                    ...
                    <span className={`absolute top-2 right-2 text-xs px-2 py-0.5 rounded border bg-[#111111]/80 ${event.is_public ? 'border-[#2A2622] text-[#B5B1AA]' : 'border-[#F59E0B]/40 text-[#F59E0B]'}`}>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs bg-[#1A1A1A] border border-[#2A2622] text-[#B5B1AA] px-2 py-0.5 rounded">{event.category || 'General'}</span>
                      <span className="text-xs text-[#7C7A74]">{event.media_count || 0} files</span>
                    </div>
                    <p className="text-xs text-[#7C7A74] mt-2">{event.club_name || 'Club'} - {event.user_role}</p>
                  </div>
                </div>
              </Link>
            ))}
